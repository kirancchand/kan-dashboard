...
import { ListUser,LISTVILLAGEUSERDATA } from './api';
...
const fetchDataReq = async (reqData:any) => {
  ...
  try {
     const response = await fetch(LISTVILLAGEUSERDATA, {
...
function SerialNoFunc(rowdata:any){
  ...
  <span className="badge bg-info-subtle  text-info">{rowdata.row.original.vId}</span>
  </div>
}

function NameFunc(rowdata:any){
  return <div>
  <div>{rowdata.row.original.firstName} {rowdata.row.original.lastName}</div>
...
function LocalBodyColumnFunc(rowdata:any){
  return <div>
  <div>{rowdata.row.original.area}</div>
  <span className="badge bg-success-subtle  text-success">{rowdata.row.original.localbody_type}</span>
  </div>
}
