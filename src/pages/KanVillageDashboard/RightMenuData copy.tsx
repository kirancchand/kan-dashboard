...
import { SearchAllUnit,INSERTUPLOADUSER,GETDISTRICT,GETLOCALBODYBYDISTRICTANDTYPE,GETWARDSBYLOCALBODY,GETVILLAGEDISTRICT,GETVILLAGELOCALBODYBYDISTRICTANDTYPE,GETBRANCHBYLOCALBODY } from './api';
...
const RightMenuData = (props: any) => {
          ...
          localBodyTypes: {
            ...
            [localBodyType]: {
              ...
              localBodies: localBodies.localBodies.reduce((acc: Record<string, LocalBodyData>, body: any) => {
                acc[body.area] = { loaded: false, wards: [] };
                return acc;
              }, {})
            }
          }
  ...
  const loadWards = async (district: string, localBodyType: string, area: string): Promise<void> => {
    const key = `${district}-${localBodyType}-${area}-wards`;
    try {
      setLoading(prev => ({ ...prev, [key]: true }));
      ...
      setTimeout(() => {
        ...
        wards.forEach((ward: Ward) => {
          newKeys.push(getWardKey(district, localBodyType, area, ward.ward_no));
        });
