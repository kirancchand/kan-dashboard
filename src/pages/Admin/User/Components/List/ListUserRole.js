...
export default function ListUserRole(props) {
  console.log(props);
  ...
  async function fetchAllUser(tempJsonData) {
    await http({
      method: 'POST',
      url: listUser,
      data:tempJsonData
  ...
  const editFunc = row => {
    props.handleFunc(row);
  };
  function actionFunc(cell, row, rowIndex, formatExtraData) {
    return (
      <div>
        <Button type="button" onClick={() => editFunc(row)}>
          edit
        </Button>
      </div>
    );
