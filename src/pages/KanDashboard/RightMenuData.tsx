...
import { SearchAllUnit,INSERTUPLOADUSER,GETDISTRICT,GETLOCALBODYBYDISTRICTANDTYPE,GETWARDSBYLOCALBODY } from './api';
...
import WardDropdown from "./Formcomponents/WardDropdown";
import { 
  Button, 
  Card, 
  CardBody, 
...
const API = {
  ...
  getWards: async (district: string, localBodyType: string, localBodyName: string): Promise<Ward[]> => {
    await new Promise(resolve => setTimeout(resolve, 250));
    
    const wardCount = Math.floor(Math.random() * 20) + 5;
    return Array.from({ length: wardCount }, (_, i) => ({
      ward_no: (i + 1).toString(),
      ward_name: `${localBodyName} Ward ${i + 1}`
    }));
  }
};
...
const RightMenuData = (props: any) => {
  ...
  const getWardKey = (district: string, localBodyType: string, localBodyName: string, wardNo: string): string => 
    `ward-${district}-${localBodyType}-${localBodyName}-${wardNo}`;

  // Get all keys for different levels
  const getLocalBodyWardKeys = (district: string, localBodyType: string, localBodyName: string): string[] => {
    const keys: string[] = [];
    if (data[district]?.localBodyTypes[localBodyType]?.localBodies[localBodyName]?.loaded) {
      data[district].localBodyTypes[localBodyType].localBodies[localBodyName].wards.forEach(ward => {
        keys.push(getWardKey(district, localBodyType, localBodyName, ward.ward_no));
      });
    }
    return keys;
  };
  ...
  const getLocalBodyAllKeys = (district: string, localBodyType: string, localBodyName: string): string[] => {
    ...
    keys.push(...getLocalBodyWardKeys(district, localBodyType, localBodyName));
    return keys;
  };
  ...
  const handleWardChange = (district: string, localBodyType: string, localBodyName: string, ward: Ward): void => {
    const wardKey = getWardKey(district, localBodyType, localBodyName, ward.ward_no);
    ...
    setSelectedItems(prev => {
      ...
      if (newSet.has(wardKey)) {
        ...
        // If no wards are selected, deselect local body
        const wardKeys = getLocalBodyWardKeys(district, localBodyType, localBodyName);
        if (!wardKeys.some(key => newSet.has(key))) {
          newSet.delete(localBodyKey);
        }
      } else {
        newSet.add(wardKey);
        // If all wards are selected, select local body
        const wardKeys = getLocalBodyWardKeys(district, localBodyType, localBodyName);
        if (wardKeys.every(key => newSet.has(key) || key === wardKey)) {
  ...
  useEffect(() => {
    ...
    selectedItems.forEach(key => {
      if (key.startsWith('district-')) {
        ...
        const parts = key.replace('ward-', '').split('-');
        const district = parts[0];
        const localBodyType = parts[1];
        const localBodyName = parts.slice(2, -1).join('-'); // Handle names with dashes
        const wardNo = parts[parts.length - 1]; // Last part is always ward number
        
        // Find ward details from data
        const allWards = data[district]?.localBodyTypes[localBodyType]?.localBodies[localBodyName]?.wards || [];
        const wardData = allWards.find(w => String(w.ward_no) === String(wardNo));
        
        if (wardData) {
          newSelected.wards.push({
            district,
            localBodyType,
            localbody_name: localBodyName,
            ward_no: wardData.ward_no,
            ward_name: wardData.ward_name,
            key: `${district}-${localBodyType}-${localBodyName}-${wardData.ward_no}`
          });
        }
      }
    });
  ...
  const loadLocalBodies = async (district: string, localBodyType: string): Promise<void> => {
    ...
    try {
      setLoading(prev => ({ ...prev, [key]: true }));
      const response = await fetch(GETLOCALBODYBYDISTRICTANDTYPE, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
          ...
          localBodyTypes: {
            ...
            [localBodyType]: {
              ...
              localBodies: localBodies.localBodies.reduce((acc: Record<string, LocalBodyData>, body: any) => {
                acc[body.localbody_name] = { loaded: false, wards: [] };
                return acc;
              }, {})
            }
          }
  ...
  const loadWards = async (district: string, localBodyType: string, localBodyName: string): Promise<void> => {
    const key = `${district}-${localBodyType}-${localBodyName}-wards`;
    try {
      setLoading(prev => ({ ...prev, [key]: true }));
      ...
      // Auto-select wards when expanding a local body
      setTimeout(() => {
        const newKeys: string[] = [];
        wards.forEach((ward: Ward) => {
          newKeys.push(getWardKey(district, localBodyType, localBodyName, ward.ward_no));
        });
      ...
      switch (type) {
        ...
        case 'localBody':
          if (localBodyType && localBodyName && 
            ...
            await loadWards(district, localBodyType, localBodyName);
          }
          break;
      }
    }
                                                        ...
                                                        <div key={`${district}-${localBodyType}-${localBodyName}`} className="border-start border-1 ps-3 mb-2" style={{borderColor: '#f8f9fa'}}>
                                                          <div className="d-flex align-items-center mb-2">
                                                            <Button
                                                              ...
                                                              disabled={loading[`${district}-${localBodyType}-${localBodyName}-wards`]}
                                                            >
                                                              {loading[`${district}-${localBodyType}-${localBodyName}-wards`] ? (
                                                                <Loader2 className="spinner-border-sm" size={16} />
                                                          ...
                                                          {expanded[`body-${district}-${localBodyType}-${localBodyName}`] && bodyData.loaded && (
                                                            <div className="ms-4">
                                                              {bodyData.wards.map((ward) => (
                                                                <div key={`${localBodyName}-${ward.ward_no}`} className="d-flex align-items-center mb-1">
                                                                  <input
                                                                    type="checkbox"
                                                                    id={`ward-${localBodyName}-${ward.ward_no}`}
                                                                    checked={selectedItems.has(getWardKey(district, localBodyType, localBodyName, ward.ward_no))}
                                                                    onChange={() => handleWardChange(district, localBodyType, localBodyName, ward)}
                                                                    className="form-check-input me-2"
                                                                  />
                                                                  <label htmlFor={`ward-${localBodyName}-${ward.ward_no}`} className="small text-muted mb-0 user-select-none">
                        ...
                        <Col lg={12}>
                          <Card>
                            ...
                            <CardBody>
                              ...
                              {/* Selected Wards */}
                              {props.selected.wards.length > 0 && (
                                <div className="mb-4">
                                  <h6 className="text-muted">
                                    Wards ({props.selected.wards.length})
                                  </h6>
                                  <div className="bg-warning bg-opacity-10 p-3 rounded border-start border-4 border-warning" style={{maxHeight: '300px', overflowY: 'auto'}}>
                                    {props.selected.wards.map((item: any, index: any) => (
                                      <div key={index} className="small text-warning-emphasis mb-2">
                                        <div className="fw-medium">Ward {item.ward_no}: {item.ward_name}</div>
