...
import { fetchGetCakeWithCommentsByCakeId } from '../../pages/Services/Api'
...
function CakeReviewTable() {
  ...
  const [cakeReview, setCakereview] = useState<AnalyticsItem[]>([]);
  ...
  const fetchData = async (reqData: any) => {
    ...
    try {
      const response = await axios.post(fetchGetCakeWithCommentsByCakeId, reqData, {
        headers: {
      ...
      setCakereview(response.data); // update state
      setTotalCount(100);
    } catch (err: any) {
  ...
  const columns = useMemo(
    () => [

      {
        id: "user_id",
        header: () => HeaderFunc(),
        accessorKey: "comments_id",
        enableColumnFilter: false,
      },
      {
        header: "Cake Name",
        accessorKey: "f_cake_name",
        enableColumnFilter: false,
        sortDescFirst: false,
        enableSorting: true,
...
export default CakeReviewTable
