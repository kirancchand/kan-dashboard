...
import { fetchGetAllCake } from "../../pages/Services/Api"
...
function CakeTable(){
 ...
 const fetchData = async (reqData:any) => {
  ...
  try {
     const response = await fetch(fetchGetAllCake, {
  ...
  return (
    <React.Fragment >
         ...
         <div style={{padding:'50px',marginTop:'40px'}}>
             <Link to='/add-cake'> <Button color="warning"  style={{margin:'20px',padding:'10px 20px',}}>+ Add</Button></Link> 
             <Card>
      <CardBody>
       <TableContainer 
        columns={(columns || [])}
